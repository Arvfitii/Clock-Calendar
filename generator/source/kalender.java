/* autogenerated by Processing revision 1283 on 2023-10-08 */
...
public class kalender extends PApplet {


 public void setup(){
  /* size commented out by preprocessor */;
  loadInfo();
  noiseDetail(12,0.65f);
  loadTemps();
  generateNoise();
  print("done");
  drawText();
  ...
  for (int i = 0; i < pixels.length; i++) {
    ...
    if(dist > 475){
      if(dist > 490){
        ...
        val = max(pow((dist-475)/15,0.7f),val);
      }
    ...
    pixels[i] = color(val*255*temp,val*255*pow(1-abs(temp-0.5f)*2,2),val*255*(1-temp));
...
float minTemp = 243.15f;
float maxTemp = 313.15f;
 ...
 public float interpolateValue(float[] positions, float[]values, float pos){
 // println(pos);
   int upper = 0;
   if(pos < 0){
     pos += 1;
 ...
 public void drawText(){
  ...
  for(int i=0; i<months.length;i++){
    ...
    float ang = (firstDay/365.0f)*PI*2 - PI/2;
 ...
 public void drawDays(){
      ...
      for(int i=0; i<365; i++){
        ...
        float ang = (PApplet.parseFloat(i)/365)*PI*2 - PI/2;
        if(ang < 0) ang += PI*2;
        if(ang > PI*2) ang -= PI*2;
        float x = cos(ang)*475;
        float y = sin(ang)*475;
 ...
 public void loadInfo(){
  ...
  name = parts[0];
  lat = PApplet.parseFloat(parts[1]);
  lon = PApplet.parseFloat(parts[2]);
}
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "kalender" };
    if (passedArgs != null) {
